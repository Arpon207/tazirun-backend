import DataModel from "../../../models/Areas/thana/ThanaModel.js";
...
import {AreaListService} from "../../../services/areas/AreaListService.js";

// Create
export const CreateThana = async (req, res) => {
    const result = await CreateWithUniqueService(req, DataModel);
    res.json(result);
}

// Update
export const UpdateThana = async (req, res) => {
    const result = await UpdateService(req, DataModel);
    res.json(result);
}

// District List
export const ThanaList = async (req, res) => {
    const SearchRegx = { "$regex": req.params.searchKeyword, "$options": "i" };
    const SearchArray = [{name: SearchRegx}];
    const result = await AreaListService(req, DataModel, SearchArray);
    res.json(result);
}

// Details
export const ThanaDetails = async (req, res) => {
    const result = await DetailsByIdService(req, DataModel);
    res.json(result);
}

// DropDown
export const ThanaDropDown = async (req, res) => {
    const result = await DropdownService(req, DataModel, {_id: 1, name: 1});
    res.json(result);
}

// Delete
export const ThanaDelete = async (req, res) => {

    const id = new ObjectId(req.params['id']);

    const checkReturnAssociate = await CheckAssociateService({divisionId: id}, ReturnsProductModel);
    const checkSalesAssociate = await CheckAssociateService({divisionId: id}, SalesModel);
