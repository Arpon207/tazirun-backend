...
const CACHE_PREFIX = 'all_brand:';

export const AllProductDetailByBrandService = async (Request, DataModel, MatchQuery, SearchArray, JoinStage1, JoinStage2, UnwindBrandStage1, UnwindBrandStage2, Projection) => {
    ...
    try{
        ...
        const brandId = MatchQuery.brandId.toString();

        // Create unique cache key based on all parameters
        const cacheKey = `${CACHE_PREFIX}${brandId}_${searchValue}_${pageNo}_${perPage}`;

        // 1️⃣ Try cache first
        const cachedData = await cacheHelper.get(cacheKey);
        if (cachedData) {
            console.log(`✅ Cache HIT for brand: ${brandId}, page: ${pageNo}`);
            return {
                status: "success",
        ...
        console.log(`🔍 Cache MISS for brand: ${brandId}, page: ${pageNo}, querying database...`);
        ...
        if (data && (data[0]?.Total?.length > 0 || data[0]?.Rows?.length > 0)) {
            cacheHelper.set(cacheKey, data, CACHE_TTL)
                .then(() => console.log(`💾 Cached data for brand: ${brandId}, page: ${pageNo}`))
        ...
        console.error("AllProductDetailByBrandService Error:", e);
        ...
        try {
            ...
            if (staleData) {
                console.log(`🔄 Using stale cache for brand: ${brandId} due to error`);
                return {
                    status: "success",
